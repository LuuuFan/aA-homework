...
class MetaCorgiSnacks
  ...
  def self.define_snack(name)
    define_method(name) do
      ...
      info = @snack_box.send("get_#{name}_info", @box_id)
      tastiness = @snack_box.send("get_#{name}_tastiness", @box_id)
      #copy from solution, don't understand why we need split "_" then join. I used debugger to check every time when define_snack from initialize, grep captured the exact name we need like 'bone', 'kibble'... but without this, the function won't work.
