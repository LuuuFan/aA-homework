...
def make_change(amount, coins = [25, 10, 5, 1])
  ...
  if amount == 0 || coins.empty?
    return coins.empty? ? [amount] : best_change
  elsif !coins.empty? && correct_coin.first != 1
    (amount / correct_coin.first).times do
      best_change << correct_coin.first
    end
    return best_change
  elsif amount == coins.first || (coins.length >= 2 && amount < coins[-2])
    (amount / correct_coin.first).times do
      best_change << correct_coin.first
    end
    return best_change
  ...
  copy_coins = coins.deep_dup
  if !copy_coins.empty? && static_amount >= copy_coins.first
    # debugger
    biggest_coin = copy_coins.shift
    (1..static_amount / biggest_coin).to_a.reverse.each do |time|
      change = []
      time.times do
        # debugger
        change << biggest_coin
      end
      change += (make_change(static_amount - change.inject(0, :+), copy_coins))
      best_change.length > change.length ? best_change = change : best_change
    end
  end
  if !coins.empty? && static_amount > coins[2] #&& static_amount < coins.first
    coins.shift
    ...
    (1..static_amount / biggest_coin).to_a.reverse.each do |time|
      ...
      change += make_change(static_amount - change.inject(0, :+), coins)
      best_change.length > change.length ? best_change = change : best_change
    end
  end
  best_change.length > change.length ? best_change = change : best_change
...
# this file will be upload to Git
